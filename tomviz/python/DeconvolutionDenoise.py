...
def deconv_admm(g, psf, mu, is_canceled=None, max_iter=50):
    ...
    while cov > tol and itr < max_iter:
        ...
        cost = (mu / 2) * np.sum((g.ravel() - conv_2d(f, psf).ravel()) ** 2) + np.sum(
            np.sqrt(vx.ravel() ** 2 + vy.ravel() ** 2)
        )
        ...
        rnorm_old = rnorm
        rnorm = np.sum(
            np.sqrt((ux.ravel() - vx.ravel()) ** 2 + (uy.ravel() - vy.ravel()) ** 2)
        )
...
def der_t(ux, uy):
    dxt_ux = np.concatenate(
        (np.reshape(ux[:, -1] - ux[:, 0], [-1, 1]), -np.diff(ux, 1, 1)), 1
    )
    dyt_uy = np.concatenate(
        (np.reshape(uy[-1, :] - uy[0, :], [1, -1]), -np.diff(uy, 1, 0)), 0
    )
...
def deconv_admm_NN(g, psf, mu, rho_r, rho_o, tol, alpha, beta, max_iter):
    ...
    while cov > tol and itr < max_iter:
        ...
        dxt_ux, dyt_uy = der_t(ux, uy)
        dxt_yx, dyt_yy = der_t(yx, yy)
        tmp1 = mu * HtG + np.fft.fft2(
            rho_r * (dxt_ux + dyt_uy) - (dxt_yx + dyt_yy) + rho_o * t - q
        )
        ...
        print(
            "Iter = %d\tConvegence = %6.4e\tCost = %6.4e\trho_r = %4.2f\trho_o = %4.2f"
            % (itr, cov, cost, rho_r, rho_o)
        )
...
def gauss(sz, w):
    ...
    if l == 1:
        ...
        X, Y = np.meshgrid(x, y)
        out = np.exp(-((Y - sz[0] // 2) ** 2) / (2 * w**2)) * np.exp(
            -((X - sz[1] // 2) ** 2) / (2 * w**2)
        )
    elif l == 3:
        y = np.arange(0, sz[0], 1)
        ...
        out = (
            np.exp(-((Y - sz[0] // 2) ** 2) / (2 * w**2))
            * np.exp(-((X - sz[1] // 2) ** 2) / (2 * w**2))
...
def conv_mat_direct(im_sz, psf):
    ...
    for row in range(im_sz[0]):
        for col in range(im_sz[1]):
            ...
            row_locs2 = np.arange(
                0, -cent[0] + row + psf_sz[0] - im_sz[0], 1, dtype="int"
            )
            row_locs3 = np.arange(
                np.fmax(-cent[0] + row, 0),
                np.fmin(-cent[0] + row + psf_sz[0], im_sz[0]),
                1,
                dtype="int",
            )
            ...
            col_locs2 = np.arange(
                0, -cent[1] + col + psf_sz[1] - im_sz[1], 1, dtype="int"
            )
            col_locs3 = np.arange(
                np.fmax(-cent[1] + col, 0),
                np.fmin(-cent[1] + col + psf_sz[1], im_sz[1]),
                1,
                dtype="int",
            )
    ...
    H = scipy.sparse.coo_matrix(
        (full_data.ravel(), (full_i_ind.ravel(), full_j_ind.ravel())), (im_tot, im_tot)
    )
    return H
...
def conv_mat_dilation(scale, lr_im_sz, psf):
    ...
    for lr_row in range(lr_im_sz[0]):
        for lr_col in range(lr_im_sz[1]):
            ...
            row_locs1 = np.arange(
                -cent[0] + hr_row + hr_im_sz[0], hr_im_sz[0], 1, dtype="int"
            )
            row_locs2 = np.arange(
                0, -cent[0] + hr_row + psf_sz[0] - hr_im_sz[0], 1, dtype="int"
            )
            row_locs3 = np.arange(
                np.fmax(-cent[0] + hr_row, 0),
                np.fmin(-cent[0] + hr_row + psf_sz[0], hr_im_sz[0]),
                1,
                dtype="int",
            )
            row_locs = np.concatenate((row_locs1, row_locs3, row_locs2))
            ...
            col_locs1 = np.arange(
                -cent[1] + hr_col + hr_im_sz[1], hr_im_sz[1], 1, dtype="int"
            )
            col_locs2 = np.arange(
                0, -cent[1] + hr_col + psf_sz[1] - hr_im_sz[1], 1, dtype="int"
            )
            col_locs3 = np.arange(
                np.fmax(-cent[1] + hr_col, 0),
                np.fmin(-cent[1] + hr_col + psf_sz[1], hr_im_sz[1]),
                1,
                dtype="int",
            )
            col_locs = np.concatenate((col_locs1, col_locs3, col_locs2))
    ...
    H = scipy.sparse.coo_matrix(
        (full_data.ravel(), (full_i_ind.ravel(), full_j_ind.ravel())),
        (lr_im_tot, hr_im_tot),
    )
    return H
...
def sp_conv_mat(scale, lr_im_sz, psf, conv_med=["dilation", "dilation"]):
    if conv_med == ["dilation", "dilation"]:
        ...
        if scale[0] * scale[1] > 1:
            ...
            H_row = conv_mat_dilation(
                [scale[0], 1], [lr_im_sz[0], hr_im_sz[1]], row_psf
            )
            if conv_med[1] == "dilation":
                col_psf = np.ones((1, 1))
            else:  # averaging
                col_psf = np.ones((1, scale[1]), dtype="float") / scale[1]
            H_col = conv_mat_dilation(
                [1, scale[1]], [lr_im_sz[0], lr_im_sz[1]], col_psf
            )
...
def sp_conv_mat_v1(scale, lr_im_sz, psf, conv_med=["dilation", "dilation"]):
    ...
    if conv_med == ["dilation", "dilation"]:
        ...
        if scale[0] > 1:
            ...
            H_row = conv_mat_dilation(
                [scale[0], 1], [lr_im_sz[0], lr_im_sz[1]], row_psf
            )
            H = H_row @ H
        return H
    ...
    for i in range(max_iter):
        ...
        for k in range(10):
            ...
            Q = (
                0.5 * np.sum((H @ w - y) ** 2)
                + grad.T @ (x_new - w)
                + 1 / (2 * a) * np.linalg.norm(x_new - w) ** 2
            )
            if L_new <= Q:
                break
            else:
...
def deconv_apg_bm3d_poisson(im, psf, scale, mu, conv_med, max_iter, is_canceled=None):
    ...
    for i in range(max_iter):
        ...
        for k in range(10):
            ...
            x_new_trans = bm3d.bm3d(
                np.reshape(w_new_trans / w_max, hr_im_sz), 1.0 / w_max
            )
...
class DeconvolutionDenoise(tomviz.operators.CancelableOperator):
    ...
    def transform(
        ...
        dataset,
        probe=None,
        selected_scalars=None,
        method="APG_BM3D",
        fast_axis_scanning="dilation",
        ...
        if probe_scalars is None:
            raise Exception(
                "There are no scalars named 'amplitude' in the selected probe dataset."
            )
        ...
        for name in selected_scalars:
            ...
            for output_slice_index, (
                ...
                w, scaled_slice, loss, rdiff = deconv(
                    scalars_slice,
                    psf,
                    scale,
                    mu,

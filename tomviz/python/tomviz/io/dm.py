...
This function is based on the Python code by Peter Ercius, NCEM
- Berkeley National Lab.
openNCEM - https://github.com/ercius/openNCEM
...
class FileDM:
    ...
    xSize, ySize, zSize : list
        ...
        same as the shape attribute for an ndarray.
    zSize2 : list
    ...
    dataOffset : list
        The starting byte number for each dataset. This can provide fast access
        directly to the data if needed by seeking to this byte number.
    dataShape : list
    ...
    thumbnail : bool
        ...
        If true then this object is always skipped in methods and it is assumed
        that the user wants to skip this. Can retrive this thumbnail using
    ...
    origin : list
        ...
        Be careful, this value is actually meant to be *scaled* by the scale
        before being used. See ncempy.io.dmReader() for proper handling of this
    ...
    def __init__(self, filename, verbose=False, on_memory=True):
        ...
        if isinstance(filename, str):
            ...
            raise TypeError('Filename is supposed to be a string/pathlib.Path')
        ...
        self.dataOffset = []
        self.dataShape = []  # 1-4. Total # of dimensions (like numpy.ndim)
        self.allTags = {}
        ...
        self._curGroupAtLevelX = np.zeros((self._maxDepth,), dtype=np.int8)
        self._curGroupNameAtLevelX = ''  # set the name of the root group
        # track tag number at the current level
        self._curTagAtLevelX = np.zeros((self._maxDepth,), dtype=np.int8)
        self._curTagName = ''  # string of the current tag
        # type which changes for DM3 and DM4
        # self._specialType = np.dtype('>u4')
        # self._dmType = None
        # self._endianType = None

        # Temporary variables to keep in case a tag entry
        # shows useful information in an array
        self._scale_temp = 0
        self._origin_temp = 0
    ...
    def _validDM(self):
        ...
        self._dmType = self.fromfile(self.fid, dtype=np.dtype('>u4'),
                                     count=1)[0]
        ...
        if self._endianType != 1:
            # print('File is not written Little Endian (PC) format
            # and can not be read by this program.')
            raise IOError('File is not written Little Endian (PC) \
            format and can not be read by this program.')
    ...
    def _readTagGroup(self):
        ...
        aa = self.fromfile(self.fid, dtype=[('IsOpenSorted', '2<i1'),
                           ('nTags', self._specialType)], count=1)
        nTags = aa['nTags'][0]
    ...
    def _readTagType(self):
        ...
        encodedType = self.fromfile(self.fid, dtype=self._specialType,
                                    count=1)[0]  # big endian
    ...
    def _encodedTypeSize(self, encodedType):
        """Return the # of bytes in a datatype for the encodings used by DM.
        Constants for the different encoded datatypes used in DM files follows:
            SHORT   = 2
            LONG    = 3
            USHORT  = 4
            ULONG   = 5
    ...
    def _readStructTypes(self):
        ...
        """
        # this is not needed
        _ = self.fromfile(self.fid, count=1, dtype=self._specialType)[0]
        ...
        for ii in range(0, nFields):
            ...
            aa = self.fromfile(self.fid, dtype=self._specialType, count=2)
    ...
    def _readNativeData(self, encodedType):
        ...
        if self._v:
            print('_readNativeData: encodedType == {} and \
                   val = {}'.format(encodedType, val))
    ...
    def _readArrayData(self, arrayTypes):
        ...
        if self._v:
            print('_readArrayData: arraySize, arrayTypes = \
                  {}, {}'.format(arraySize, arrayTypes))
        ...
        if self._v:
            print('_readArrayData: arraySize, \
                  itemSize = {}, {}'.format(arraySize, itemSize))
        ...
        if self._curTagName == 'Data':
            ...
            self.seek(self.fid, bufSize.astype('<u8'), 1)
            arrOut = 'Data unread. Encoded type = {}'.format(encodedType)
        # set an upper limit on the size of array
        # that will be read in as a string
        elif bufSize < 1e3:
            # treat as a string
            for encodedType in arrayTypes:
    ...
    def _storeTag(self, curTagName, curTagValue):
        """Builds the full tag name and key/value pair as text.
        Also calls another function to catch useful tags and values.
        Also saves all tags in a dictionary.
        ...
        if self._v:
            print('_storeTag: curTagName, \
            curTagValue = {}, {}'.format(curTagName, curTagValue))
        totalTag = self._curGroupNameAtLevelX + '.' + '{}'.format(curTagName)
    ...
    def _catchUsefulTags(self, totalTag, curTagName, curTagValue):
        ...
        This is separate from _storeTag so that it is easy to find and modify.
    ...
    def writeTags(self, new_folder_path_for_tags=None):
        ...
        in the same directory (or a user definable directory)
        and with a the same name as the DM file.
    ...
    def getDataset(self, index):
        ...
        Notes
        -----
            Most DM3 and DM4 files contain a small "thumbnail" as the
            first dataset written as RGB data. This function ignores that
            dataset if it exists. To retrieve the thumbnail use the
            getThumbnail() function.

            The pixelOrigin returned is not actually the start of the
            coordinates. The start of the energy axis for EELS (for example)
            will be pixelSize * pixelOrigin. dmReader() returns the correct
            coordinates. The correct origin is: pixelSize * pixelOrigin and
            be careful about the sign as it seems some datasets might use
            -pixelOrigin in the previous equation.

        Parameters
        ----------
            index : int
                The number of the data set to retrieve ignoring the thumbnail.
                If a thumbnail exists then index = 0 actually corresponds to
                the second data set in a DM file.
        ...
        """
        # The first dataset is usually a thumbnail. Test for this and
        # skip the thumbnail automatically
    ...
    def getMemmap(self, index):
        ...
        This is very useful for very large datasets to avoid loading the entire
        data set into memory. No meta data is returned.
        ...
        -------
            : numpy.memmap
                ...
                The file will continue to be open as long as the memmap is open
                Delete the memmap to close the file.
        """
        # The first dataset is usually a thumbnail.
        # Test for this and skip the thumbnail automatically
        if self.numObjects == 1:
...
def dmReader(filename, dSetNum=0, verbose=False, on_memory=True):
    ...
    ----------
        ...
        on_memory : bool, default True
            Whether to use the on_memory option of fileDM.
            Usually provides much faster data reading.
    ...
    -------
        : dict
            ...
            'coords' contains the coordinate axes with the proper origin
            and scale applied (i.e. the energy loss axis for EELS data)
    ...
    -------
        Load all data from a single image dm3 file and display it
            ...
            >>> im0 = dm.dmReader('filename.dm3')
             #show the single image from the data file
            >>> plt.imshow(im0['data'])
    """
    # Open the file

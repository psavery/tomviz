...
PipelineProxy::PipelineProxy()
{
  QObject::connect(&ModuleManager::instance(), &ModuleManager::dataSourceAdded,
                   [this](DataSource* ds) {
                     QObject::connect(ds, &DataSource::dataChanged,
  ...
  QObject::connect(&ModuleManager::instance(), &ModuleManager::moduleAdded,
                   [this](Module* module) {
                     QObject::connect(module, &Module::renderNeeded,
                                      [this]() { this->syncToPython(); });
                   });
  ...
  QObject::connect(&ModuleManager::instance(),
                   &ModuleManager::visibilityChanged,
                   [this]() { this->syncToPython(); });
...
void PipelineProxy::syncToPython()
{
  if (!m_syncToPython) {
    return;
  }
...
void PipelineProxy::syncViewsToPython()
{
  Python python;
  auto tomvizState = python.import("tomviz.state");
  if (!tomvizState.isValid()) {
...
std::string PipelineProxy::serialize()
{
  QJsonObject state;
...
void PipelineProxy::load(const std::string& state,
...
std::string PipelineProxy::operatorsJson()
{
  vtkNew<vtkImageData> data;
  data->SetDimensions(2, 2, 2);
  data->AllocateScalars(VTK_INT, 1);
...
std::string PipelineProxy::serializeOperator(const std::string& path,
                                             const std::string& id)
{
...
void PipelineProxy::updateOperator(const std::string& path,
...
std::string PipelineProxy::serializeModule(const std::string& path,
                                           const std::string& id)
{
  auto p = QString::fromStdString(path);
  auto i = QString::fromStdString(id);
...
void PipelineProxy::updateModule(const std::string& path,
                                 const std::string& state)
{
  auto p = QString::fromStdString(path);
  auto json = QByteArray::fromStdString(state);
...
std::string PipelineProxy::serializeDataSource(const std::string& path,
                                               const std::string& id)
{
  auto p = QString::fromStdString(path);
  auto i = QString::fromStdString(id);
...
void PipelineProxy::updateDataSource(const std::string& path,
                                     const std::string& state)
{
  auto p = QString::fromStdString(path);
  auto json = QByteArray::fromStdString(state);
...
void PipelineProxy::modified(std::vector<std::string> opPaths,
...
std::string PipelineProxy::addModule(const std::string& dataSourcePath,
...
std::string PipelineProxy::addOperator(const std::string& dataSourcePath,
                                       const std::string& dataSourceId,
...
std::string PipelineProxy::addDataSource(const std::string& dataSourceState)
{
...
void PipelineProxy::removeModule(const std::string& modulePath,
                                 const std::string& dataSourceId,
...
void PipelineProxy::removeDataSource(const std::string& dataSourcePath,
                                     const std::string& dataSourceId)
{
  auto p = QString::fromStdString(dataSourcePath);
  auto i = QString::fromStdString(dataSourceId);
...
void PipelineProxy::enableSyncToPython()
{
  m_syncToPython = true;
}

void PipelineProxy::disableSyncToPython()
{
  m_syncToPython = false;
}

void PipelineProxy::pausePipeline(const std::string& dataSourcePath)
{
  auto p = QString::fromStdString(dataSourcePath);
...
void PipelineProxy::resumePipeline(const std::string& dataSourcePath)
{
  auto p = QString::fromStdString(dataSourcePath);
  auto ds = findDataSource(p);
  if (ds == nullptr) {
...
void PipelineProxy::executePipeline(const std::string& dataSourcePath)
{
  auto p = QString::fromStdString(dataSourcePath);
  auto ds = findDataSource(p);
  if (ds == nullptr) {
...
bool PipelineProxy::pipelinePaused(const std::string& dataSourcePath)
{
  auto p = QString::fromStdString(dataSourcePath);
  auto ds = findDataSource(p);
  if (ds == nullptr) {
...
PipelineProxyBase* PipelineProxyFactory::create()
{
  return new PipelineProxy();
}

void PipelineProxyFactory::registerWithFactory()
{
  PythonFactory::instance().setPipelineProxyFactory(new PipelineProxyFactory);
}
